...
const VIDEO_FILTERS = [
  ...
  },
  {
    ...
    label: "Bühne",
    matches: (text) =>
      text.includes("bühne") || text.includes("laufsteg") || text.includes("traverse"),
  },
  {
    id: "tent",
    label: "Zelte & Kuppeln",
    matches: (text) => text.includes("zelt") || text.includes("kuppel") || text.includes("dome"),
  },
  {
    id: "corporate",
    label: "Firmenevents",
    matches: (text) =>
      text.includes("firmen") ||
      text.includes("gala") ||
      text.includes("launch") ||
      text.includes("konferenz") ||
      text.includes("kongress") ||
      text.includes("messe") ||
  ...
  },
  {
    ...
    matches: (text) =>
      text.includes("ton") ||
      text.includes("licht") ||
      text.includes("projektion") ||
      text.includes("laser") ||
      text.includes("beschallung"),
  },
  {
    id: "esports",
    label: "E-Sport",
    matches: (text) => text.includes("e-sport") || text.includes("esport") || text.includes("pubg"),
  },
];
...
function ProjectVideoCard({ video, index }) {
  return (
    <article className="group relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.055] p-3 shadow-2xl shadow-black/20 backdrop-blur transition duration-300 hover:-translate-y-1 hover:border-violet-300/35 hover:bg-white/[0.075]">
      <div
        className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(139, 92, 246,0.16),transparent_34%)] opacity-0 transition duration-300 group-hover:opacity-100"
        aria-hidden="true"
      />
      ...
      <div className="relative p-5">
        ...
        <ul className="mt-5 flex flex-wrap gap-2" aria-label="Im Video gezeigte Gewerke">
...
export default function VideoGallery({ videos }) {
  ...
  const filteredVideos = useMemo(() => {
    const selectedFilter =
      VIDEO_FILTERS.find((filter) => filter.id === activeFilter) ?? VIDEO_FILTERS[0];
    ...
    <>
      <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.045] px-4 py-3 text-sm text-slate-300 shadow-xl shadow-black/10 backdrop-blur">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2 lg:justify-end" aria-label="Filter für Projektvideos">
