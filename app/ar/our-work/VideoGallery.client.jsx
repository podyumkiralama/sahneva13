...
const VIDEO_FILTERS = [
  ...
  },
  {
    id: "led",
    label: "شاشة LED",
    matches: (text) => text.includes("led") || text.includes("شاشة"),
  },
  {
    id: "stage",
    label: "مسرح",
    matches: (text) =>
      text.includes("مسرح") || text.includes("منصة") || text.includes("تراس"),
  },
  {
    id: "tent",
    label: "خيمة / قبة",
    matches: (text) => text.includes("خيمة") || text.includes("قبة"),
  },
  {
    id: "corporate",
    label: "فعاليات مؤسسية",
    matches: (text) =>
      text.includes("مؤسسي") ||
      text.includes("حفل") ||
      text.includes("إطلاق") ||
  ...
  {
    id: "esports",
    label: "رياضات إلكترونية",
    matches: (text) => text.includes("إلكتروني") || text.includes("pubg"),
  },
];
    ...
    <article className="group relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.055] p-3 shadow-2xl shadow-black/20 backdrop-blur transition duration-300 hover:-translate-y-1 hover:border-blue-300/35 hover:bg-white/[0.075]">
      ...
      <div className="relative overflow-hidden rounded-[1.4rem] border border-white/10 bg-black">
        ...
        <div className="pointer-events-none absolute right-4 top-4 inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/45 px-3 py-1.5 text-xs font-black text-white backdrop-blur">
          ...
          فيديو {String(index + 1).padStart(2, "0")}
        </div>
      </div>
      <div className="relative p-5">
        ...
        <ul className="mt-5 flex flex-wrap gap-2" aria-label="الخدمات في هذا الفيديو">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-black text-white">
              {filteredVideos.length}/{videos.length} فيديو
            </span>
          </div>
          <div className="flex flex-wrap gap-2 lg:justify-end" aria-label="فلاتر مشاريع الفيديو">
