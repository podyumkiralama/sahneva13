 ...
 *
 * Dil secici (`components/i18n/LanguageSwitcher.client.jsx`) bu tabloyu okur.
 * hreflang HENUZ OKUMUYOR — sayfalar kendi `buildLanguageAlternates`
 * cagrilarini kullanmaya devam ediyor; gecis ayri bir adim olarak planlandi
 * (asagida `getTranslations` notuna bakin). Tablo yine de ikisinin ORTAK
 * kaynagi olacak sekilde tasarlandi ve verisi sayfalarin bugun bastigi
 * hreflang'dan birebir uretildi.
 *
...
export function findGroup(pathname) {
  ...
  const byTranslation = PAGE_GROUPS.find((group) =>
 ...
 *
 * Not: bu yardimci su an sayfalara BAGLI DEGIL — sayfalar hreflang'i hala kendi
 * `buildLanguageAlternates` cagrilariyla uretiyor. Sayfalarin bu yardimciya
 * gecirilmesi ayri ve kontrollu bir adim olarak planlandi (24 grup / 97 sayfa).
 *
 * O gecis yapilana kadar tablo ile sayfa cagrilari ayrisabilir: `npm run seo:html`
 * hreflang'in KARSILIKLILIGINI ve x-default tutarliligini denetler, ama bu
 * tablodan sapmayi denetlemez. Bir grubu burada degistirirseniz ilgili sayfalarin
 * `alternates` bloklarini elle ayni hale getirin.
 */
export function getTranslations(pathname) {
  return findGroup(pathname)?.translations ?? null;
}
