 ...
 *
 * Alan sozlesmesi:
 *   org       -> kurum adi (gorunen, tam hali)
 *   shortName -> dar yuzeylerde kullanilacak kisa ad (istege bagli). Kisaltmayi
 *                cagiran tarafta metin keserek uretmeyin; ad burada yazili olmali.
...
export const CLIENT_REFERENCES = [
  ...
  {
    // Kurum adi proje sayfasinda yazili DEGIL; isletmeden teyit alindi
    // (31 Tem 2026). Proje sayfasi yalnizca "Millet Bahcesi Hangar'da protokol
    // etkinligi" diyor — bu kaydi degistirirken oradaki metin de guncellenmeli.
    org: "Gençlik ve Spor Bakanlığı",
    work: "Amatör futbol kulüplerine nakdi destek programı töreni",
...
/**
 * Verilen kurum adlarini listedeki kayitlarla eslestirir, verilen sirayla dondurur.
 *
 * Bilinmeyen ad build'i KIRAR. `org` metinleri duzenlenebilir (unvan, yil, turnuva
 * adi degisebilir); sessiz bir `undefined` filtrelenip gecseydi cagiran sayfada
 * referans sayisi fark edilmeden azalirdi — ne hata ne uyari.
 */
export function getClientReferences(orgs) {
  return orgs.map((org) => {
    const entry = CLIENT_REFERENCES.find((item) => item.org === org);

    if (!entry) {
      throw new Error(
        `getClientReferences: "${org}" lib/clientReferences.js icinde bulunamadi.`,
      );
    }
