 ...
 */
export async function appendMessage(conversation, { role, text, file = null }) {
  const message = { role, text: text ?? "", at: Date.now(), ...(file ? { file } : {}) };
...
export async function getMessages(id, since = 0) {
  ...
  const messages = raw
    ...
    .filter((entry) => entry && (typeof entry.text === "string" || entry.file));
...
/**
 * Sohbeti ve mesajlarını kalıcı olarak siler. Dosyalar çağıran tarafta
 * ayrıca siliniyor; burası yalnızca Redis kayıtlarını temizler.
 */
export async function deleteConversation(id) {
  ...
  const commands = [
    ["DEL", conversationKey(id)],
    ["DEL", messagesKey(id)],
    ["HDEL", SUMMARY_KEY, id],
  ];

  // Kişi eşleştirme kaydı yalnızca hâlâ bu sohbeti gösteriyorsa silinsin;
  // aynı numarayla sonradan açılmış bir sohbetin bağı kopmasın.
  if (conversation.contactKey) {
    const current = await command(["GET", contactIndexKey(conversation.contactKey)]);
    if (current === id) {
      commands.push(["DEL", contactIndexKey(conversation.contactKey)]);
    }
  }
