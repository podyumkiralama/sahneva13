...
const isLikelyPlayerUrl = (value, pageUrl) => {
  const raw = String(value ?? "").trim();
  if (!raw) return false;
  if (normalizeUrlForCompare(raw) === normalizeUrlForCompare(pageUrl)) return false;
...
const normalizeVideo = (video, pageUrl) => {
  ...
  const validContentLoc = contentLoc && isLikelyMediaUrl(contentLoc) ? contentLoc : null;
  const validPlayerLoc = isLikelyPlayerUrl(playerLoc, pageUrl) ? playerLoc : null;
