...
export const BLOCKED_REGIONAL_CITY_SLUGS = new Set(["istanbul"]);

export function isBlockedRegionalCity(slug) {
  return BLOCKED_REGIONAL_CITY_SLUGS.has(slug);
}

export function getIndexableRegionalCities() {
  return REGIONAL_CITIES.filter((city) => !isBlockedRegionalCity(city.slug));
}

export function getRegionalCity(slug) {
  if (isBlockedRegionalCity(slug)) return undefined;
...
export function getFeaturedRegionalCities() {
  ...
  return getIndexableRegionalCities().filter((city) => featured.has(city.slug));
}
