...
import { findTranslationGroup } from "@/lib/i18n/pageEquivalents";
...
export function buildAlternatesForPath(pathname) {
  // findGroup DEGIL: o, yedek eslesmelerini de sayar ve yalnizca yedek olarak
  // gecen bir adresi ait olmadigi grubun cevirisi gibi gosterebilirdi.
  const group = findTranslationGroup(pathname);

  if (!group) {
    throw new Error(
      `hreflang: "${pathname}" lib/i18n/pageEquivalents.js tablosunda gercek ceviri ` +
        `olarak kayitli degil. Yalnizca "fallbacks" icinde geciyorsa hreflang'a giremez ` +
        `(yedek, esdegerlik beyani degildir). Sayfanin gercek bir dil grubu varsa onu ` +
        `translations'a ekleyin; yoksa buildAlternatesForPath kullanmayin.`
    );
  }
