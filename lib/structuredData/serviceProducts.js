...
export function buildServiceProductSchema({ slug, locale = "tr-TR" }) {
  ...
  const disableOfferAnchors = entry.slug === "/ses-isik-sistemleri";
  ...
  const productNodes = entry.products.map((product, index) => {
    ...
    const node = {
      ...
      offers: {
        "@type": "Offer",
        "@id": offerId,
        url: disableOfferAnchors ? pageUrl : `${pageUrl}#teklif-${product.sku}`,
  ...
  const aggregateOffer =
    prices.length > 0
      ? {
          ...
          availability: "https://schema.org/InStock",
          url: disableOfferAnchors ? pageUrl : `${pageUrl}#teklifler`,
        }
      : undefined;
  ...
  const baseOfferNodes = baseOfferEntries
    .map((offer, index) => {
      ...
      return {
        ...
        url: disableOfferAnchors ? pageUrl : `${pageUrl}#fiyat-${index + 1}`,
        availability: "https://schema.org/InStock",
        price,
        priceCurrency,
        priceValidUntil:
